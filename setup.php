...
// 1) Accessible from localhost only — no remote access to setup wizard
// === DOČASNĚ zakomentováno pro webhosting instalaci bez SSH ===
// Bezpečnost zajišťuje .setup-allowed marker (viz check níže).
...
// 2) If .env already exists, the application is installed — block reinstall
if (is_file(__DIR__ . '/.env')) {
    http_response_code(403);
    die('Application already installed. Delete .env to reinstall.');
}

// 3) Require .setup-allowed marker file — prevents accidental re-runs and race conditions.
//    Admin must create this empty file before running setup; setup deletes it on success.
if (!is_file(__DIR__ . '/.setup-allowed')) {
    http_response_code(403);
    die('Setup is not enabled. Create empty file .setup-allowed in the project root first.');
}

// 4) Lightweight CSRF guard — config.php may not be loadable yet, so we can't use the
//    main csrf_verify() helper. This standalone guard is intentionally minimal.
...
// ── env_safe: strip CR/LF/NULL to prevent newline injection into .env ──────
...
// ── Krok 3: IP + API + zápis .env ─────────────────────────
    ...
    if (empty($_SESSION['setup'])) {
        ...
        // env_safe() applied to every user-supplied value to prevent newline injection (SEC-006)
...
if ($success) {
    // Delete the marker first so a concurrent request can't sneak through
...
<?php else: ?>
    ...
    <?php if ($errors): ?>
    <div class="errors" role="alert"><strong><?= s('errors_title') ?></strong><ul>
    ...
    <form method="post" action="setup.php?step=1&lang=<?= $lang ?>">
        ...
        <label for="db_host"><?= s('db_host') ?></label>
        <input type="text" id="db_host" name="db_host" value="<?= htmlspecialchars($_POST['db_host'] ?? 'localhost') ?>" autocomplete="off">
        <p class="hint"><?= s('db_host_hint') ?></p>

        <label for="db_name"><?= s('db_name') ?></label>
        <input type="text" id="db_name" name="db_name" value="<?= htmlspecialchars($_POST['db_name'] ?? 'gpx_manager') ?>" required autocomplete="off">
        <p class="hint"><?= s('db_name_hint') ?></p>

        <label for="db_user"><?= s('db_user') ?></label>
        <input type="text" id="db_user" name="db_user" value="<?= htmlspecialchars($_POST['db_user'] ?? 'root') ?>" required autocomplete="username">

        <label for="db_pass"><?= s('db_pass') ?></label>
        <input type="password" id="db_pass" name="db_pass" placeholder="" autocomplete="current-password">
    ...
    <form method="post" action="setup.php?step=2&lang=<?= $lang ?>">
        ...
        <label for="admin_user"><?= s('admin_user') ?></label>
        <input type="text" id="admin_user" name="admin_user" value="<?= htmlspecialchars($_POST['admin_user'] ?? 'admin') ?>" required autofocus autocomplete="username">

        <label for="admin_pass"><?= s('admin_pass') ?></label>
        <input type="password" id="admin_pass" name="admin_pass" placeholder="<?= s('admin_pass_hint') ?>" required autocomplete="new-password">
        <p class="hint"><?= s('admin_pass_hint') ?></p>

        <label for="admin_pass2"><?= s('admin_pass2') ?></label>
    ...
    <form method="post" action="setup.php?step=3&lang=<?= $lang ?>">
        ...
        <label for="admin_ips"><?= s('admin_ips') ?></label>
        <input type="text" id="admin_ips" name="admin_ips"
               value="<?= htmlspecialchars('127.0.0.1,::1,' . ($_SERVER['REMOTE_ADDR'] ?? '')) ?>" autocomplete="off">
        <p class="hint"><?= sprintf(s('admin_ips_hint'), htmlspecialchars($_SERVER['REMOTE_ADDR'] ?? '')) ?></p>

        <label for="tf_key"><?= s('tf_key') ?> <span class="opt"><?= s('optional') ?></span></label>
        <input type="text" id="tf_key" name="tf_key" placeholder="" autocomplete="off">
        <p class="hint"><?= s('tf_hint') ?></p>

        <label for="mapy_key"><?= s('mapy_key') ?> <span class="opt"><?= s('optional') ?></span></label>
        <input type="text" id="mapy_key" name="mapy_key" placeholder="" autocomplete="off">
        <p class="hint"><?= s('mapy_hint') ?></p>

        <label for="mapillary_tok"><?= s('mapillary_tok') ?> <span class="opt"><?= s('optional') ?></span></label>
        <input type="text" id="mapillary_tok" name="mapillary_tok" placeholder="" autocomplete="off">

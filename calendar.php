...
/* ===== Theme (zachováno pro public_access kompatibilitu) ===== */
...
<section class="mx-auto max-w-7xl px-4 sm:px-6 pt-6">
    ...
    <h1 class="font-[Manrope] text-3xl md:text-4xl font-extrabold tracking-tight text-forest-700 dark:text-sand-100 flex items-center gap-3">
        <i data-lucide="calendar-heart" class="w-8 h-8 text-terracotta-500"></i>
...
<section class="mx-auto max-w-7xl px-4 sm:px-6 mt-6 mb-12">
    <div class="card-outdoor p-5 overflow-x-auto">
        <div class="cal-grid">
            ...
            $monthNames = t('months_short');
            if (!is_array($monthNames)) $monthNames = ['','Led','Úno','Bře','Dub','Kvě','Čvn','Čvc','Srp','Zář','Říj','Lis','Pro'];
            ...
            $dayLabels = t('days_short');
            if (!is_array($dayLabels)) $dayLabels = ['Po','Út','St','Čt','Pá','So','Ne'];
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const i18n = {
        days: <?= json_encode(t('days_full')) ?>,
        noActivity: <?= json_encode(t('tooltip_no_activity')) ?>,
        singular: <?= json_encode(t('track_singular')) ?>,
        few: <?= json_encode(t('track_few')) ?>,
        plural: <?= json_encode(t('track_plural')) ?>
    };

...
function validate_gpx_file(string $path) {
    ...
    $xml = safe_load_gpx_string($xmlText);
    if (!$xml) return 'Soubor není platný XML (případně obsahuje zakázané DTD/ENTITY deklarace).';
    ...
    if ($existing) {
        ...
        if ($mode === 'overwrite') {
            ...
            try {
                ...
                // Výpočet centroidu z bounds a uložení do DB
                $boundsRaw = $gpx['bounds'] ?? null;
                if ($boundsRaw !== null) {
                    $boundsArr = json_decode((string)$boundsRaw, true);
                    if (is_array($boundsArr)) {
                        $minLat = $boundsArr['minlat'] ?? ($boundsArr['minLat'] ?? null);
                        $maxLat = $boundsArr['maxlat'] ?? ($boundsArr['maxLat'] ?? null);
                        $minLon = $boundsArr['minlon'] ?? ($boundsArr['minLon'] ?? null);
                        $maxLon = $boundsArr['maxlon'] ?? ($boundsArr['maxLon'] ?? null);
                        if ($minLat !== null && $maxLat !== null && $minLon !== null && $maxLon !== null) {
                            ...
                            $pdo->prepare("UPDATE tracks SET centroid_lat = ?, centroid_lon = ? WHERE id = ?")
                                ->execute([$cLat, $cLon, (int)$existing['id']]);
                        }
                    }
                }
...
<section class="mx-auto max-w-7xl px-4 sm:px-6 pt-6">
    ...
    <h1 class="font-[Manrope] text-3xl md:text-4xl font-extrabold tracking-tight text-forest-700 dark:text-sand-100 flex items-center gap-3">
        <i data-lucide="upload" class="w-8 h-8 text-terracotta-500" aria-hidden="true"></i>
...
<input id="fileInput" type="file" accept=".gpx" multiple aria-label="<?= htmlspecialchars(t('import_file_label', 'Vybrat GPX soubory')) ?>">

...
<!-- STAT CARDS GRID -->
<section class="mx-auto max-w-7xl px-4 sm:px-6 mt-6 anim-fade-up anim-fade-up-2">
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        <div class="card-outdoor p-4">
            <div class="flex items-center gap-1.5 text-xs uppercase tracking-wider text-forest-700/60 dark:text-sand-100/60">
                <i data-lucide="ruler" class="w-3.5 h-3.5"></i> <?= htmlspecialchars(t('th_distance')) ?>
            </div>
        ...
        <div class="card-outdoor p-4">
            <div class="flex items-center gap-1.5 text-xs uppercase tracking-wider text-forest-700/60 dark:text-sand-100/60">
                <i data-lucide="trending-up" class="w-3.5 h-3.5"></i> <?= htmlspecialchars(t('th_ascent')) ?>
            </div>
        ...
        <div class="card-outdoor p-4">
            <div class="flex items-center gap-1.5 text-xs uppercase tracking-wider text-forest-700/60 dark:text-sand-100/60">
                <i data-lucide="trending-down" class="w-3.5 h-3.5"></i> <?= htmlspecialchars(t('th_descent')) ?>
            </div>
        ...
        <div class="card-outdoor p-4">
            <div class="flex items-center gap-1.5 text-xs uppercase tracking-wider text-forest-700/60 dark:text-sand-100/60">
                <i data-lucide="clock" class="w-3.5 h-3.5"></i> <?= htmlspecialchars(t('th_duration')) ?>
            </div>
        ...
        <div class="card-outdoor p-4">
            <div class="flex items-center gap-1.5 text-xs uppercase tracking-wider text-forest-700/60 dark:text-sand-100/60">
                <i data-lucide="gauge" class="w-3.5 h-3.5"></i> <?= htmlspecialchars(t('th_speed_avg')) ?>
            </div>
        ...
        <div class="card-outdoor p-4">
            <div class="flex items-center gap-1.5 text-xs uppercase tracking-wider text-forest-700/60 dark:text-sand-100/60">
                <i data-lucide="mountain-snow" class="w-3.5 h-3.5"></i> <?= htmlspecialchars(t('th_elev_max')) ?>
            </div>
...
<section class="mx-auto max-w-7xl px-4 sm:px-6 mt-6">
    <div id="map" class="w-full h-[60vh] min-h-[400px] rounded-2xl overflow-hidden shadow-card border border-sand-200 dark:border-forest-800"></div>
</section>
...
window.gpxDetailData = {
    trackId:      <?= json_encode($track ? (int)$track['id'] : null) ?>,
    dateStart:    <?= json_encode($track['date_start'] ?? null) ?>,
    weatherLat:   <?= json_encode($_weatherLat ?? null) ?>,
    weatherLon:   <?= json_encode($_weatherLon ?? null) ?>,
    fileNameEnc:  "<?= rawurlencode($track['filename'] ?? '') ?>",
    fileNamePlain:"<?= h($track['filename'] ?? '') ?>",
    trackTitle:   "<?= h(($track['track_name'] ?: $track['filename']) ?? 'profil') ?>",
    gpxUrl:       <?= json_encode(gpx_url($track['filename'] ?? '')) ?>,
    dbAvgSpeed:   <?= json_encode(round($track['speed_avg']       ?? 0, 3)) ?>,
    dbAvgMoving:  <?= json_encode(round($track['speed_avg_total'] ?? 0, 3)) ?>,
    garminColor:  "<?= h($track['color'] ?? '') ?>",
    apiKeys: {
        tf:        <?= json_encode(defined('TF_API_KEY') ? TF_API_KEY : '') ?>,
        mapycom:   <?= json_encode(defined('MAPYCOM_API_KEY') ? MAPYCOM_API_KEY : '') ?>,
        mapillary: <?= json_encode(defined('MAPILLARY_TOKEN') ? MAPILLARY_TOKEN : '') ?>
    },
    i18n: {
        elev_label_elev:  <?= json_encode(t('elev_label_elev')) ?>,
        elev_label_dist:  <?= json_encode(t('elev_label_dist')) ?>,
        slope_steep_down: <?= json_encode(t('slope_steep_down')) ?>,
        slope_mild_down:  <?= json_encode(t('slope_mild_down')) ?>,
        slope_flat:       <?= json_encode(t('slope_flat')) ?>,
        slope_steep_up:   <?= json_encode(t('slope_steep_up')) ?>,
        colorful_tooltip: <?= json_encode(t('colorful_tooltip')) ?>
    }
};
</script>
        ...
        .then(data => {
            ...
            tracks.forEach(t => {
                ...
                card.innerHTML =
                    ...
                    '</div>' +
                    '<div class="font-[Manrope] font-semibold text-forest-700 dark:text-sand-100 group-hover:text-terracotta-500 transition-colors line-clamp-1">' + (t.track_name || t.filename) + '</div>' +
                    '<div class="mt-1 text-sm text-forest-700/70 dark:text-sand-100/70">' +
                        '<span class="stat-num">' + t.distance_km + '</span> km · ↑<span class="stat-num">' + t.ascent + '</span> m · ' + formatDuration(t.duration) +
                    '</div>' +

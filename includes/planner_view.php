...
window.gpxPlannerData = {
    ...
    i18n: {
        ...
        <?php // Omezení a přichycení bodu — Mapy.com to vracejí od prosince 2025 ?>
        restrHead:      <?= js_safe_json(t('planner_restr_head',      'Omezení na trase')) ?>,
        pointWord:      <?= js_safe_json(t('planner_point_word',      'bod')) ?>,
        restrNoEntry:   <?= js_safe_json(t('planner_restr_no_entry',  'zákaz vstupu')) ?>,
        restrLimited:   <?= js_safe_json(t('planner_restr_limited',   'omezený vstup (povolenka, poplatek)')) ?>,
        restrPedestrian:<?= js_safe_json(t('planner_restr_pedestrian','pěší zóna')) ?>,
        ...
        <?php // Zapnutý radar = jen prohlížení srážek, plán se needituje ?>
        radarViewOn:     <?= js_safe_json(t('planner_radar_view_on',      'Vrstva srážek je zapnutá — plánovač je teď jen prohlížeč radaru. Body se nepřidávají ani nepřesouvají.')) ?>,
        radarViewBlocked:<?= js_safe_json(t('planner_radar_view_blocked', 'Body se teď nepřidávají — je zapnutá vrstva Aktuální srážky. Vypni ji ve výběru vrstev a plánuj dál.')) ?>,

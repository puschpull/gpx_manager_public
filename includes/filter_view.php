...
<section class="mx-auto max-w-7xl px-4 sm:px-6 pt-6">
    ...
    <h1 class="font-[Manrope] text-3xl md:text-4xl font-extrabold tracking-tight text-forest-700 dark:text-sand-100 flex items-center gap-3">
        <i data-lucide="wand-2" class="w-8 h-8 text-terracotta-500"></i>
...
window.gpxFilterData = {
    ...
    gpxUrl:       <?= json_encode($track ? gpx_url($track['filename']) : null) ?>,
    fileName:     <?= json_encode($track ? $track['filename'] : null) ?>,
    trackName:    <?= json_encode($track ? ($track['track_name'] ?: $track['filename']) : null) ?>,

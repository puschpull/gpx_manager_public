...
<section class="mx-auto max-w-7xl px-4 sm:px-6 mt-2 anim-fade-up anim-fade-up-3">
    <div class="flex flex-wrap items-center gap-2">
        <a href="<?= gpx_chip_url(['fav_only'=>'','activity'=>'','year'=>'','date_from'=>'','date_to'=>'']) ?>" class="chip-filter" <?= (!$activeFav && !$activeActivity && !$activeYear && !$activeQ) ? 'aria-current="true"' : '' ?>>
            <i data-lucide="list" class="w-3.5 h-3.5" aria-hidden="true"></i> <?= htmlspecialchars(t('chip_all')) ?>
        </a>
        <a href="<?= gpx_chip_url(['fav_only'=>$activeFav?'':'1']) ?>" class="chip-filter" <?= $activeFav ? 'aria-current="true"' : '' ?>>
            <i data-lucide="star" class="w-3.5 h-3.5" aria-hidden="true"></i> <?= htmlspecialchars(t('chip_fav')) ?>
        </a>
        <a href="<?= gpx_chip_url(['date_from'=>$activeYear?'':$thisYearStart, 'year'=>$activeYear?'':date('Y')]) ?>" class="chip-filter" <?= $activeYear ? 'aria-current="true"' : '' ?>>
            <i data-lucide="calendar-days" class="w-3.5 h-3.5" aria-hidden="true"></i> <?= htmlspecialchars(t('chip_thisyear')) ?>
        </a>
        <a href="<?= gpx_chip_url(['activity'=>$activeActivity==='Pěšky'?'':'Pěšky']) ?>" class="chip-filter" <?= $activeActivity==='Pěšky' ? 'aria-current="true"' : '' ?>>
            <i data-lucide="footprints" class="w-3.5 h-3.5" aria-hidden="true"></i> <?= htmlspecialchars(t('chip_walk')) ?>
        </a>
        <a href="<?= gpx_chip_url(['activity'=>$activeActivity==='Turistika'?'':'Turistika']) ?>" class="chip-filter" <?= $activeActivity==='Turistika' ? 'aria-current="true"' : '' ?>>
            <i data-lucide="mountain" class="w-3.5 h-3.5" aria-hidden="true"></i> <?= htmlspecialchars(t('chip_hiking')) ?>
        </a>
        <a href="<?= gpx_chip_url(['activity'=>$activeActivity==='Auto'?'':'Auto']) ?>" class="chip-filter" <?= $activeActivity==='Auto' ? 'aria-current="true"' : '' ?>>
            <i data-lucide="car" class="w-3.5 h-3.5" aria-hidden="true"></i> <?= htmlspecialchars(t('chip_auto')) ?>
        </a>
        <a href="<?= gpx_chip_url(['activity'=>$activeActivity==='Kolo'?'':'Kolo']) ?>" class="chip-filter" <?= $activeActivity==='Kolo' ? 'aria-current="true"' : '' ?>>
            <i data-lucide="bike" class="w-3.5 h-3.5" aria-hidden="true"></i> <?= htmlspecialchars(t('chip_bike')) ?>
        </a>
        ...
        <span class="ml-auto flex items-center gap-2 text-sm">
            <a href="index-legacy.php<?= !empty($_SERVER['QUERY_STRING']) ? '?'.$_SERVER['QUERY_STRING'] : '' ?>"
               ...
               title="<?= htmlspecialchars(t('legacy_table_hint')) ?>">
                <i data-lucide="table-2" class="w-4 h-4" aria-hidden="true"></i>
        ...
        <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <?php foreach ($tracks as $i => $tr): ?>
                <?php
                    ...
                    $diff  = $tr['difficulty'] ?? '';
                    $act   = $tr['activity_type'] ? activity_type_label($tr['activity_type']) : '';
                    $color = $tr['color'] ?? '';
                ?>
                <?php
                ...
                <article class="card-outdoor overflow-hidden group relative" style="animation: fadeInUp .45s var(--ease-out-soft) <?= ($i % 9) * 50 ?>ms backwards">
                    ...
                    <button type="button"
                        ...
                        <i data-lucide="star" class="w-5 h-5" <?= $isFav ? 'fill="currentColor"' : '' ?> aria-hidden="true"></i>
                    </button>
...
document.addEventListener('click', async function (ev) {
    ...
    try {
        ...
        const r = await fetch('api_toggle_favorite.php', {
            method: 'POST',

    ...
    <div class="flex items-start justify-between gap-3 flex-wrap">
        <div>
            ...
            <h1 class="font-[Manrope] text-3xl md:text-4xl font-extrabold tracking-tight text-forest-700 dark:text-sand-100 flex items-center gap-3">
                <i data-lucide="table-2" class="w-8 h-8 text-terracotta-500"></i>
...
<!-- Topbar -->
<div class="topbar">
    <button id="sidebarToggle" class="sidebar-toggle active" title="<?= t('title_filters') ?>">
        <?= t('btn_filter') ?>
    </button>
...
window.gpxChartData = {
    ...
    distance:  <?= json_encode($chart_distance  ?? []) ?>,
    count:     <?= json_encode($chart_count     ?? []) ?>,
    ascent:    <?= json_encode($chart_ascent    ?? []) ?>,
    avg_speed: <?= json_encode($chart_avg_speed ?? []) ?>
};
</script>

<!-- JS moduly -->
<script src="js/index-data.js" defer></script>
...
function runBulkAction() {
    ...
    if (action === 'set_category' || action === 'remove_category') {
        const label = action === 'set_category' ? <?= json_encode(t('prompt_category')) ?> : <?= json_encode(t('prompt_remove_cat')) ?>;
        value = prompt(label);
    ...
    if (action === 'set_color') {
        value = prompt(<?= json_encode(t('prompt_color')) ?>);
        if (value === null || value.trim() === '') return;
        value = value.trim();
    }

    if (action === 'delete') {
        if (!confirm(<?= json_encode(t('confirm_bulk_delete')) ?> + ' ' + ids.length + ' ' + <?= json_encode(t('confirm_bulk_suffix')) ?>)) return;
    }
    ...
    .then(data => {
        if (data.success) {
            ...
            alert(<?= json_encode(t('msg_error')) ?> + (data.error || <?= json_encode(t('msg_unknown_error')) ?>));
        }
    })
    .catch(err => {
        alert(<?= json_encode(t('msg_comm_error')) ?> + err.message);
    });
}

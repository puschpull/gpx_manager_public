...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_access_config'])) {
    ...
    $allLangs  = ['cs','en','de','sk','es','fr','pl','it'];
    $allPages  = ['stats','calendar','heatmap','map_search','nearby','filter','compare','settings','photos'];

    $themes = array_values(array_intersect($_POST['allowed_themes'] ?? [], $allThemes));
    $langs  = array_values(array_intersect($_POST['allowed_langs']  ?? [], $allLangs));
    $pages  = array_values(array_intersect($_POST['allowed_pages']  ?? [], $allPages));
...
$actIn = implode(',', array_map(fn($c) => $pdo->quote($c), $activityCats));
$withActivity = (int)$pdo->query("
    SELECT COUNT(DISTINCT tc.track_id)
    FROM track_categories tc
    JOIN categories c ON c.id = tc.category_id
    WHERE c.name IN ($actIn)
")->fetchColumn();
...
<section class="mx-auto max-w-7xl px-4 sm:px-6 pt-6">
    ...
    <h1 class="font-[Manrope] text-3xl md:text-4xl font-extrabold tracking-tight text-forest-700 dark:text-sand-100 flex items-center gap-3">
        <i data-lucide="shield" class="w-8 h-8 text-terracotta-500"></i>
    ...
    <?php if (isset($_GET['saved']) && $_GET['saved'] === 'access'): ?>
        <div class="mt-3 inline-flex items-center gap-2 px-3 py-1.5 rounded-md bg-forest-100 dark:bg-forest-700 text-forest-700 dark:text-sand-100 text-sm">
            <i data-lucide="check-circle" class="w-4 h-4"></i> Konfigurace přístupu uložena
        </div>
    <?php endif; ?>
</section>
...
<div class="admin-grid">
        ...
        <ul class="tool-list">
            ...
            <li>
                <a href="photo_import.php">
                    <span>📥 Lokální import fotek</span>
                </a>
                <div class="tool-desc">Skenuje adresář na PC, vybere a importuje fotky hromadně (bez uploadu přes prohlížeč)</div>
            </li>
            <li>
    ...
    <div class="admin-card">
        ...
        <ul class="tool-list">
            ...
            <li>
                <a href="login.php?logout=1" onclick="return confirm(<?= json_encode(t('confirm_logout')) ?>);">
...
<?php
$cfgThemes = get_app_config('allowed_themes', $allThemes);
$cfgLangs  = get_app_config('allowed_langs',  ['cs','en','de','sk','es','fr','pl','it']);
$cfgPages  = get_app_config('visible_pages',  ['stats','calendar','heatmap','map_search','nearby','filter','compare','settings']);
...
<div style="margin:24px 0;">
    <h2 style="font-size:17px;margin-bottom:12px;color:var(--text-color);">🔐 Konfigurace přístupu pro návštěvníky</h2>
    <form method="POST" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:16px;">

        <div class="admin-card">
            <h3>🎨 Dostupné vzhledy</h3>
        ...
        <div class="admin-card">
            <h3>📄 Viditelné stránky pro návštěvníky</h3>
            <p style="font-size:11px;color:var(--text-muted);margin-bottom:10px;">
                Admin vidí vždy vše. Odškrtnuté stránky přesměrují návštěvníky na hlavní stránku.
            </p>
            <div style="display:flex;flex-direction:column;gap:6px;">
        ...
        <div style="grid-column:1/-1;">
            <button type="submit" name="save_access_config" class="btn" style="font-size:14px;padding:10px 24px;">
                💾 Uložit nastavení přístupu
            </button>
            <p style="font-size:11px;color:var(--text-muted);margin-top:6px;">
                Minimálně 1 téma a 1 jazyk musí zůstat aktivní (fallback na Classic / Čeština).
            </p>
        </div>
...
<div class="card-outdoor p-5 mt-6">
    <h3 class="font-[Manrope] font-semibold text-forest-700 dark:text-sand-100 mb-1 flex items-center gap-2">
        <i data-lucide="folder-cog" class="w-5 h-5 text-terracotta-500"></i>
        Konfigurace cest k uploads/
    </h3>
    <p class="text-sm text-forest-700/65 dark:text-sand-100/65 mb-4">
        Pokud chceš sdílet adresář <code>uploads/</code> mezi více instancemi (např. číst produkční fotky a GPX z testovací subdomény), nastav zde absolutní cestu.
        Ponech prázdné pro použití lokálního <code>./uploads/</code>.
    </p>
    <?php if (isset($_GET['saved']) && $_GET['saved'] === 'uploads'): ?>
        <div class="mb-3 inline-flex items-center gap-2 px-3 py-1.5 rounded-md bg-forest-100 dark:bg-forest-700 text-forest-700 dark:text-sand-100 text-sm">
            <i data-lucide="check-circle" class="w-4 h-4"></i> Konfigurace cest uložena
        </div>
    <?php endif; ?>
    ...
    <form method="post" class="space-y-4">
        ...
        <div class="pt-2 border-t border-sand-200 dark:border-forest-700 flex items-center gap-3 flex-wrap">
            <button type="submit" name="save_uploads_config" class="btn-outdoor btn-outdoor-primary">
                <i data-lucide="save" class="w-4 h-4"></i> Uložit cesty
            </button>

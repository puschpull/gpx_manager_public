...
<section class="mx-auto max-w-7xl px-4 sm:px-6 pt-6">
    ...
    <h1 class="font-[Manrope] text-3xl md:text-4xl font-extrabold tracking-tight text-forest-700 dark:text-sand-100 flex items-center gap-3">
        <i data-lucide="pencil" class="w-8 h-8 text-terracotta-500"></i>
    ...
    <div class="track-nav">
        <?php if ($prevId): ?>
            <a class="btn-outdoor btn-outdoor-ghost" href="edit.php?id=<?= $prevId ?><?= h($navQs) ?>">
                <i data-lucide="chevron-left" class="w-4 h-4"></i><?= t('nav_prev') ?>
            </a>
        <?php else: ?>
        ...
        <?php if ($nextId): ?>
            <a class="btn-outdoor btn-outdoor-ghost" href="edit.php?id=<?= $nextId ?><?= h($navQs) ?>">
                <?= t('nav_next') ?><i data-lucide="chevron-right" class="w-4 h-4"></i>
            </a>
        <?php else: ?>
            <span class="btn-outdoor btn-outdoor-ghost opacity-40 pointer-events-none">
                <?= t('nav_next') ?><i data-lucide="chevron-right" class="w-4 h-4"></i>
...
<div class="meta">
    ...
    <?php if ($flash): ?>
        <div class="flash <?= str_starts_with($flash, 'Chyba') ? 'flash-error' : 'flash-ok' ?>">
            <?= h($flash) ?>
        </div>
    <?php endif; ?>
        ...
        <div class="row">
            <div class="col">
                <button class="btn-outdoor btn-outdoor-primary" type="submit">
                    <i data-lucide="save" class="w-4 h-4"></i><?= t('save') ?>
                </button>
                <a class="btn-outdoor btn-outdoor-ghost" href="<?= h($editBackUrl) ?>">
                    <i data-lucide="arrow-left" class="w-4 h-4"></i><?= htmlspecialchars($editBackLabel) ?>
                </a>
                <a class="btn-outdoor btn-outdoor-ghost" href="detail.php?id=<?= (int)$track['id'] ?>&<?= h($navQs) ?>">
                    <i data-lucide="map" class="w-4 h-4"></i><?= t('btn_show_on_map') ?>
                </a>
            </div>
        </div>
    ...
    <!-- Smazání trasy -->
    <form method="post" action="delete.php" onsubmit="return confirm(<?= json_encode(t('confirm_delete') . ' \'' . ($track['track_name'] ?: $track['filename']) . '\'' . t('confirm_delete_end')) ?>);"

...
/* ===== Theme (zachováno pro zpětnou kompatibilitu) ===== */
...
<section class="mx-auto max-w-7xl px-4 sm:px-6 pt-6">
    ...
    <h1 class="font-[Manrope] text-3xl md:text-4xl font-extrabold tracking-tight text-forest-700 dark:text-sand-100 flex items-center gap-3">
        <i data-lucide="bar-chart-2" class="w-8 h-8 text-terracotta-500"></i>
                ...
                <div class="flex items-start gap-4">
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <a href="detail.php?id=<?= (int)$r['id'] ?>" class="mt-1 inline-flex items-center gap-1 text-sm text-forest-600 hover:text-terracotta-500 transition-colors line-clamp-1">
                            ...
                            <i data-lucide="arrow-up-right" class="w-3 h-3 shrink-0"></i>
                        </a>
                    </div>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const datasets = {
        km:        <?= json_encode($chart_km) ?>,
        count:     <?= json_encode($chart_count) ?>,
        ascent:    <?= json_encode($chart_ascent) ?>,
        avg_speed: <?= json_encode($chart_avg_speed) ?>
    };
    const labelMap = {
        km:        <?= json_encode(t('metric_distance')) ?>,
        count:     <?= json_encode(t('metric_count')) ?>,
        ascent:    <?= json_encode(t('metric_ascent')) ?>,
        avg_speed: <?= json_encode(t('metric_avg_speed')) ?>
    };
    ...
    const diffLabels = <?= json_encode($diffLabels) ?>;
    const diffValues = <?= json_encode($diffValues) ?>;
    const diffColors = <?= json_encode(array_slice($diffColors, 0, count($diffLabels))) ?>;

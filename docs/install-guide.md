...
> Version 2026-05 &nbsp;|&nbsp; [Česká verze](install-navod.md)
...
> **No Composer needed:** The application does not require `composer install` or any build tools (Node.js, etc.). It runs straight after uploading the files — even on shared hosting without SSH access.
...
### Before Running setup.php — Create the `.setup-allowed` File

BEFORE opening `setup.php` in your browser, create an empty file named `.setup-allowed` in the project root folder.

**WampServer / XAMPP (Windows):**
```
C:\wamp64\www\gpx_manager\.setup-allowed
```
Simply create an empty file — right-click in Explorer → New → Text Document, name it `.setup-allowed` (remove the `.txt` extension).

**Web Hosting (via FTP / SSH):**
```bash
touch .setup-allowed
```

> **Why?** The `.setup-allowed` file acts as a security marker. Without it, `setup.php` will refuse to run — protecting you from accidentally reinstalling an existing application. The wizard deletes it automatically after a successful installation.

### Step 1 — Database Connection

Open in your browser:
- WampServer: `http://localhost/gpx_manager/setup.php`
- Web hosting: the wizard is accessible **from localhost only** (127.0.0.1 or ::1) — run it via an SSH tunnel or locally
...
After importing the baseline schema, run the migration runner to ensure the database structure is up to date:

```bash
php migrate.php
```

The script prints the status of each migration (`OK`, `SKIP`) and the total number applied at the end.
A second run safely does nothing: `Done. 0 migration(s) applied.`

> **Web access:** `migrate.php` can also be opened in a browser, but **from localhost only** (`127.0.0.1` or `::1`). Any other IP receives HTTP 403.
...
4. Run `php migrate.php` — it applies any new migrations automatically
...
- See the detailed `.htaccess` configuration guide (localhost and web hosting, Alias and VirtualHost) in [htaccess-guide.md](htaccess-guide.md)
